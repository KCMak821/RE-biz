...
test("manual invoice snapshots, draft workflow, sent lock, overdue and void", { concurrency: false }, async () => {
  ...
  const filteredList = await request("/api/invoices?q=no-such-invoice", { token: fixture.owner }); assert.equal(filteredList.response.status, 200); assert.equal(filteredList.body.invoices.length, 0); assert.ok(filteredList.body.total >= 1);
...
test("quote conversion uses accepted snapshots and is one-to-one", { concurrency: false }, async () => {
  ...
  const filteredList = await request("/api/quotes?status=rejected", { token: fixture.owner }); assert.equal(filteredList.response.status, 200); assert.equal(filteredList.body.quotes.length, 0); assert.ok(filteredList.body.total >= 1);
...
test("invoice permissions, ownership, feature flags and suspension are enforced", { concurrency: false }, async () => {
  ...
  assert.equal((await request(`/api/invoices/${invoice.id}`, { token: fixture.operator })).response.status, 404); assert.equal((await request(`/api/invoices/${invoice.id}`, { token: fixture.ownerB })).response.status, 404); assert.equal((await request("/api/invoices", { body: invoicePayload(customer.id), method: "POST", token: fixture.viewer })).response.status, 403); assert.equal((await request(`/api/invoices/${invoice.id}`, { body: { action: "void" }, method: "PATCH", token: fixture.viewer })).response.status, 403);
