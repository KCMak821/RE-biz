...
export async function PUT(request: Request, { params }: { params: Promise<{ itemId: string }> }) {
  ...
  try {
    ...
    const result = await (await itemsCollection()).findOneAndUpdate({ _id: id, organizationId: new ObjectId(user.organization.id) }, { $set: { ...parsed.data, updatedAt: new Date() } }, { returnDocument: "after" });
...
export async function DELETE(_: Request, { params }: { params: Promise<{ itemId: string }> }) {
  ...
  try {
    ...
    if (!await canUseWorkspaceFeature(user, "quotations")) return Response.json({ message: "此工作區目前無法使用報價單功能。" }, { status: 403 });
    const result = await (await itemsCollection()).deleteOne({ _id: id, organizationId: new ObjectId(user.organization.id) });
