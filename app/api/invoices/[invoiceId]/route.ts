...
export async function PATCH(request: Request, { params }: { params: Promise<{ invoiceId: string }> }) {
  ...
  try { const user = await getCurrentUser(); if (!user) return Response.json({ message: "請先登入。" }, { status: 401 }); if (!canManageRecords(user)) return Response.json({ message: "你的角色只有檢視權限，無法更新請款單。" }, { status: 403 }); if (!await permitted(user)) return Response.json({ message: "此工作區目前無法使用請款單功能。" }, { status: 403 }); const invoice = await inWorkspace(id, user); if (!invoice) return Response.json({ message: "請款單不存在。" }, { status: 404 }); const allowed = parsed.data.action === "send" ? invoice.status === "draft" : invoice.status === "draft" || invoice.status === "sent"; if (!allowed) return Response.json({ message: "目前狀態不可進行此操作。" }, { status: 409 }); const next = parsed.data.action === "send" ? { status: "sent" as const, sentAt: new Date(), updatedAt: new Date() } : { status: "void" as const, updatedAt: new Date() }; const result = await (await invoicesCollection()).findOneAndUpdate({ _id: id, organizationId: new ObjectId(user.organization.id), status: invoice.status }, { $set: next }, { returnDocument: "after" }); return result ? Response.json({ invoice: serializeInvoice(result) }) : Response.json({ message: "請款單已被更新，請重新整理。" }, { status: 409 });
