...
export async function POST(_: Request, { params }: { params: Promise<{ quoteId: string }> }) {
  ...
  try {
    ...
    const copy: QuoteDocument = {
      companySnapshot: source.companySnapshot, createdAt: now, createdBy: source.createdBy, currency: "HKD", customerId: source.customerId,
      ...
      quoteNumber: await nextQuoteNumber(userId, issueDate), status: "draft", terms: source.terms, totalAmount: source.totalAmount,
